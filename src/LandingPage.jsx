...
const LandingPage = () => {
  ...
  const displayDrink = useMemo(() => {
    if (!drinksContent) return null;
    let randomIndex = 0;
    ...
    return drinksContent[randomIndex];
  }, [drinksContent]);

  return (
    <div className="size-full flex-grow nice-trans">
      <div className="w-full col-flex-center p-3 gap-5 lg:grid grid-cols-2 lg:gap-3">
        <div className="drinksNDescription size-full grid grid-rows-3 col-span-1 container">
          <div className="drinks-entry-container size-full rounded-t-lg row-span-2">
            <button
              className="drinks-entry-btn size-full text-white p-2 "
              onClick={() => navigate("/drinks/0")}
            >
              <span className="z-30">
                <h2 className="font-semibold text-2xl md:text-3xl lg:text-4xl text-center overflow-hidden overflow-ellipsis">
                  Take me to the booze! 🍺
                </h2>
          ...
          <div className="mission-statement p-2 bg-beerDarkOrange flex items-center text-white rounded-b-lg nice-trans">
            <p className="text-xl md:text-2xl lg:text-3xl font-hind lg:text-center font-thin">
              Your center for drinking on a budget, the last and only web-app
              you'll need for getting drunk as cheaply as you can
            </p>
          </div>
        </div>
        <div className="random-pick-card size-full col-flex-center col-span-1 bg-white p-2 container nice-trans">
          ...
          {displayDrink != null ? (
            <DrinksCard
              height={450}
              width={450}
              className={`!rounded-none !shadow-none !w-5/6`}
              index={0}
              drink={displayDrink}
            />
          ) : (
