...
const CategoriesSection = (props) =>{
    ...
    return(
        <div className={`categoriesSection ${props.className}`}>
            <button onClick={() => setShowOptions(!showOptions)} className={`w-full text-left font-semibold mb-1 flex items-center justify-between`}>
                <p>Categories</p>
                <p>{!showOptions ? <FaPlus /> : <FaMinus />}</p>
            </button>
            {showOptions&&<ul className="flex flex-col gap-1">
