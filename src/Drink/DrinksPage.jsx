...
const DrinksPage = ({ chunkIndex, chunk }) => {
  ...
  const countryCode = useMemo(
    () =>
      countries?.find((country) => country.name === drink?.origin_country)
        ?.code,
    [drink],
  );
  ...
  const MoreDetails = () => {
    ...
    return (
      <>
        <h3 className="font-medium p-1 w-full border-b border-black mb-1">
          Additional Details
        </h3>
        <TableWBody tableClassName={`drinkDetails w-full`}>
          <TableFullRow th="Total Volume">
            <td>{drink?.total_volume * drink?.pieces_per} ml</td>
          </TableFullRow>

          <TableFullRow th="Alc / Vol">
            <td>{drink?.alcohol_percent?.toFixed(1)}%</td>
          </TableFullRow>

          <TableFullRow th="Standard Drinks">
          ...
          <TableFullRow th="Container">
            <td>{drink?.container}</td>
          </TableFullRow>

          <TableFullRow th="Store">
            <td>{drink?.store}</td>
          </TableFullRow>
        </TableWBody>
      </>
  ...
  const MainDetails = () => {
    const volText = useMemo(
      () =>
        drink?.pieces_per > 1
          ? `${drink?.pieces_per} x ${drink?.total_volume} ml`
          : `${drink?.total_volume} ml`,
      [],
    );

    return (
      <>
        <h2 className="font-[sans-serif] text-3xl font-semibold text-center lg:text-left">
          {drink?.drink_name}
        </h2>
        <p className="font-[sans-serif] font-medium">{volText}</p>
        <p className="text-3xl font-medium font-[sans-serif]">
    ...
    <div className="drinkPage mb-auto size-full p-3 col-flex-center">
      <div className="w-full border-b border-black flex items-center gap-3 mb-3 pb-2 text-2xl">
        <button
          ...
          className="p-2 nice-trans hover:text-appleLightBlue"
        >
          <FaArrowLeft />
        </button>
      ...
      <div className="w-full flex flex-col items-start">
        <h3 className="text-xl font-bold mb-5 text-left font-[sans-serif]">
          Rank In Category: # {drinkRanking}
        </h3>

        {drink?.origin_country && flagUrl ? (
          <img src={flagUrl} alt={drink?.origin_country} />
        ) : (
          <p className="text-2xl font-semibold">
            Product of {drink?.origin_country}
          </p>
        )}
      </div>
      <div className="col-flex-center w-full lg:flex-row lg:w-5/6 lg:gap-5 lg:grid lg:grid-cols-2 lg:place-items-center">
        ...
        <div className="flex w-full flex-col gap-3 lg:col-span-1">
          ...
          <button
            ...
            className="card-btn nice-trans bg-appleBlue hover:bg-appleLightBlue text-white font-semibold text-xl"
          >
            See on {drink?.store}
          </button>
          <MoreDetails />
