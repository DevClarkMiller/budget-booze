...
const DrinksCard = ({
  ...
  const logoColour = useMemo(
    () => (drink.store === "LCBO" ? "text-[#174634]" : "text-beerOrange"),
    [drink],
  );
  ...
  return (
    <>
      {drink && (
        <>
          <CardWrapper
            to={!isNaN(index) && nav ? `single/${index}` : null}
            className={`drinks-card-mobile nice-trans ${className ? className : "drinks-card-responsive"} lg:flex shadow-md`}
          >
            <div className={`flippable-card-front`}>
              <div className="drink-header flex items-center gap-2 w-3/4">
                ...
                <h2 className="drink-name flex-grow text-one-line">
                  {drink.drink_name}
                </h2>
              </div>
