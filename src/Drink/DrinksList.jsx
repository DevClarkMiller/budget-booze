  ...
  return (
    <div className={`${props.className}`}>
      ...
      <div ref={btnsRef} className="flex flex-row justify-center gap-2 mb-3">
        <button
          disabled={!pageDetails?.hasPrevPage}
          className={`${pageDetails.hasPrevPage ? "page-nav-btn" : "disabled-nav-btn"} row-flex-center items-center text-2xl`}
          onClick={() => handleDecrementPage(props.id)}
        >
          <FaArrowLeft />
        </button>
        <button
          disabled={!pageDetails?.hasNextPage}
          className={`${pageDetails.hasNextPage ? "page-nav-btn" : "disabled-nav-btn"} row-flex-center items-center text-2xl`}
          onClick={() => handleIncrementPage(props.id)}
        >
          <FaArrowRight />
        </button>
      </div>
